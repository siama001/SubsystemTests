...
public class ApplicationLogicSubsystemTest {
	 ...
	 * Input: HttpRequest, HttpResponse, InitParameter("action-type")="doLogin", valid user credentials
	...
	public void SST1a() throws Exception{
		...
		/* Assert */
		assertSame("the session should contain a valid UserProfile",mockSession.getAttribute("User Profile"),mockUser);
		assertTrue( "the session should contain a PanelistForms",
                mockSession.getAttribute( "User Form" ) instanceof PanelistForms );
		assertSame("the session url should be 'messagePage?messageCode=You are now logged in. Welcome.'",
				((StubSession) mockSession).getURL(),"messagePage?messageCode=You are now logged in. Welcome.");
	 ...
	 * Expected Output: HttpSession(StubSession): Attributes:"User Profile"==null, and Session URL == "login"
	 */
	 ...
	 * a user account.
	 * Initial State: HttpSession(StubSession): blank session
	 * Input: HttpRequest(mockRequest), HttpResponse(mockResponse), InitParameter("action-type")="createPanelistAccount",
	...
	public void SST3() throws Exception{
		...
		assertSame("the session url should be 'messagePage?messageCode=Registration Successful'",((StubSession) mockSession).getURL(),"messagePage?messageCode=Registration Successful");
	}
	@Test
	/**
	 ...
	 * a user account.
	 * Initial State: HttpSession(StubSession): blank session
	 * Input: HttpRequest(mockRequest), HttpResponse(mockResponse), InitParameter("action-type")="createPanelistAccount", invalid panelist info
	 * Expected Output: StubSession.URL = "errorPage?errorCode=Registration failed. Please try again."
	 */
	public void SST4() throws Exception{
		...
		PowerMockito.verifyStatic();
		DBManager.createPanelist(newPanelist);
		assertSame("the session URL should be 'errorPage?errorCode=Registration failed. Please try again.'",((StubSession) mockSession).getURL(),"errorPage?errorCode=Registration failed. Please try again.");
	}
	@Test
	/**
	 * Sub Systems Test 5(SST5)
	 * Verify that a invalid createPanelistAccount request to the facade with an already existing ISCID
	 * will not create a user account
	 * Initial State: HttpSession(StubSession): blank session
	 * Input: HttpRequest(mockRequest), HttpResponse(mockResponse), InitParameter("action-type")="createPanelistAccount", invalid panelist info
	 * Expected Output: StubSession.URL = "errorPage?errorCode=Double Registration Exception: ISCID is already in use."
	 */
	public void SST5() throws Exception{
		...
		PowerMockito.verifyStatic();
		DBManager.createPanelist(newPanelist);
		assertSame("the session URL should be 'errorPage?errorCode=Double Registration Exception: ISCID is already in use.'",((StubSession) mockSession).getURL(),"errorPage?errorCode=Double Registration Exception: ISCID is already in use.");
	 ...
	 * Verify that a addToPanel request to the facade with invalid IDs will not add to the panel  
	 * Initial State: HttpSession(StubSession): blank session
	 * Input: HttpRequest, HttpResponse, InitParameter("action-type")="addToPanel", PanelID, PanelistID
	...
	public void SST6() throws Exception{
		...
		assertSame("the session url is 'messagePage?messageCode=Panelist has been successfully added.'",
				((StubSession) mockSession).getURL(),"messagePage?messageCode=Panelist has been successfully added.");
	...
	public void SST7() throws Exception{		
		...
		PowerMockito.verifyStatic();
		DBManager.addPanelistToPanel(555,1);
		assertSame("the session url is 'errorPage?errorCode=Error adding panelist. Please try again.'",
				((StubSession) mockSession).getURL(),"errorPage?errorCode=Error adding panelist. Please try again.");
		//verify( mockResponse, atMost( 1 ) ).sendRedirect( "login" );
	 ...
	 * Verify that a updatePanelStatus request to the facade with valid inputs will update panel status
	 * and redirect user to the expected page
	 * Initial State: HttpSession(StubSession): blank session
	 * Input: HttpRequest, HttpResponse, InitParameter("action-type")="updatePanelStatus", String status, Sting comments, int panelID
	 * Expected Output: HttpSession(StubSession): Session URL == ("messagePage?messageCode=Panel status updated.")
	 */
	public void SST8() throws Exception{
		...
		assertSame("the session url should be 'messagePage?messageCode=Panel status updated.'",
				((StubSession) mockSession).getURL(),"messagePage?messageCode=Panel status updated.");
		//verify( mockResponse, atMost( 1 ) ).sendRedirect( "login" );
	 ...
	 * Verify that a updatePanelStatus request to the facade with invalid inputs will not update panel status
	 * and redirect user to the expected page
	 * Initial State: HttpSession(StubSession): blank session
	 * Input: HttpRequest, HttpResponse, InitParameter("action-type")="updatePanelStatus", String status, Sting comments, int panelID
	 * Expected Output: HttpSession(StubSession): Session URL == ("errorPage?errorCode=Error updating panel. Please try again.")
	 */
	public void SST9() throws Exception{
		...
		controllerSpy.doPost(mockRequest, mockResponse);
		/* Assert */
		assertSame("the session url should be 'errorPage?errorCode=Error updating panel. Please try again.'",
				((StubSession) mockSession).getURL(),"errorPage?errorCode=Error updating panel. Please try again.");
		//verify( mockResponse, atMost( 1 ) ).sendRedirect( "login" );
	 ...
	 * Verify that a createPanel request to the facade with valid inputs will create a panel
	 * and redirect user to the expected page
	 * Initial State: HttpSession(StubSession): blank session
	 * Input: HttpRequest, HttpResponse, InitParameter("action-type")="updatePanelStatus", String status, Sting comments, int panelID
	 * Expected Output: HttpSession(StubSession): Session URL == ("errorPage?errorCode=Error updating panel. Please try again.")
